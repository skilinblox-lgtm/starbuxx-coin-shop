...
const BrainrotReviews = ({ brainrotId }: BrainrotReviewsProps) => {
  ...
  const [buyerUserIds, setBuyerUserIds] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Check which reviewers have orders (are clients)
      const userIds = [...new Set((data || []).map((r: any) => r.user_id).filter(Boolean))];
      if (userIds.length > 0) {
        const { data: ordersData } = await supabase.from("orders").select("user_id").in("user_id", userIds).neq("status", "cancelado");
        setBuyerUserIds(new Set((ordersData || []).map(o => o.user_id)));
      }
    };
    fetchData();
  }, [brainrotId]);
      ...
      <div className="rounded-2xl border border-border bg-card/50 backdrop-blur-sm p-5 sm:p-8">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {reviews.map((review, i) => {
            const isClient = buyerUserIds.has(review.user_id);
            return (
              <motion.div
                key={review.id}
