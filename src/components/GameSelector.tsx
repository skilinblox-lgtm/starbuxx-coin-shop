  ...
  return (
    <section id="jogos" className="bg-surface py-12 sm:py-20">
      <div className="container px-4">
        ...
        <div className="mt-8 flex gap-3 overflow-x-auto pb-2 sm:mt-12 sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:pb-0">
          {games.map((game) => (
            ...
            >
              ...
              {selectedGame.id === game.id && (
                <div className="absolute right-3 top-3 h-2.5 w-2.5 rounded-full bg-primary animate-pulse-gold sm:right-4 sm:top-4 sm:h-3 sm:w-3" />
              )}
            </button>
          ))}
        </div>
        ...
        {/* Calculator */}
        <div className="mx-auto mt-8 max-w-xl rounded-2xl border border-border bg-background p-5 shadow-[var(--shadow-card)] sm:mt-12 sm:p-8">
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-xl bg-surface p-3 sm:mt-6 sm:flex-row sm:items-end sm:justify-between sm:p-4">
            ...
            <button
              onClick={handleBuy}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground shadow-[var(--shadow-gold)] transition-all hover:brightness-110 hover:scale-105 sm:px-6 sm:py-3 sm:text-base"
            >
