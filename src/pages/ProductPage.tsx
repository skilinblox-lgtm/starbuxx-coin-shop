...
import { Star, ShieldCheck, ArrowLeft, Minus, Plus, ShoppingCart, Clock, Zap, CheckCircle, Truck, Award } from "lucide-react";
        ...
        <div className="container px-4 pb-12 pt-20 sm:pt-24">
          ...
          <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
            ...
            <motion.div
              ...
              className="flex items-center justify-center rounded-3xl border border-border bg-card p-8 sm:p-12"
            >
              {product.image_url ? (
                <img
                  src={product.image_url}
                  alt={product.name}
                  className="max-h-72 w-auto object-contain drop-shadow-2xl sm:max-h-96"
                />
              ) : (
                <div className="flex h-72 w-72 items-center justify-center rounded-2xl bg-surface text-muted-foreground">
          ...
          {reviews.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {reviews.map(r => (
                  <div key={r.id} className="rounded-2xl border border-border bg-card p-4 sm:p-5">
